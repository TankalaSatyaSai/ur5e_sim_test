...
int main(int argc, char * argv[])
{
  ...
  auto arm_group_interface = MoveGroupInterface(node, "arm_tool1");
  ...
  arm_group_interface.setMaxVelocityScalingFactor(0.40);
 
  //  Set a scaling factor for optionally reducing the maximum joint acceleration. Allowed values are in (0,1].
  arm_group_interface.setMaxAccelerationScalingFactor(0.40);
  ...
  auto moveit_visual_tools =
      moveit_visual_tools::MoveItVisualTools{ node, "ur5e_base_link", rviz_visual_tools::RVIZ_MARKER_TOPIC,
  ...
  auto const draw_trajectory_tool_path =
    [&moveit_visual_tools, jmg = arm_group_interface.getRobotModel()->getJointModelGroup("arm_tool1")](
  ...
  auto const arm_target_pose = [&node]{
    ...
    msg.header.stamp = node->now();
     msg.pose.position.x = -0.702;
     msg.pose.position.y = 0.109;
     msg.pose.position.z = 0.678;
     msg.pose.orientation.x = -0.016;
     msg.pose.orientation.y = 0.852;
     msg.pose.orientation.z = 0.109;
     msg.pose.orientation.w =  -0.512;
    return msg;
  }();
  ...
  auto const collision_object = [frame_id = arm_group_interface.getPlanningFrame(), &node, &logger] {
    ...
    primitive.dimensions[primitive.BOX_Z] = 1.2;  // Height
    ...
    box_pose.position.y = 0.22;   // Centered in y-direction
